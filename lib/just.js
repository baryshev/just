...
(function () {
	...
	var
		...
		async = (function () {
			...
			async.notParallel = function (tasks, callback) {
				callback = callback || function () {};
				if (tasks.constructor === Array) 
                                {
		...
		JUST = function (newOptions) {
			var
					...
					if (Object.prototype.toString.call(options.root) === '[object Object]') {
						try {
							...
							if (Object.prototype.toString.call(data) === '[object String]') {
								...
								return callback('Failed to load template');
							}
						} catch (e) {
							return callback(e);
						}
				...
				load = function (file, callback) {
					if (options.useCache && cache[file]) {
						if (callback) {  return callback(undefined, cache[file]); }
					} else {
						if (!loaders[file]) 
                                                {
							loaders[file] = [];
							if (callback) { loaders[file].push(callback); }
							return read(file, function (error, data) {
								if (error) {
			...
			Template.prototype.render = function (callback) {// Ogiginal
				var that = this;
				...
				load(this.file, function (error, blank) {
					if (error) 
						...
						try {
							...
							return async.parallel(that.partials, function (error) {
								var html = '', length, i;
								if (!error) 
                                                                {
